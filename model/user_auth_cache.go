...
if ARGV[9] == '0' and redis.call('EXISTS', KEYS[1]) == 0 then
  return 1
end
redis.call('HSET', KEYS[1],
  'Id', ARGV[2], 'Email', ARGV[3],
  'Status', ARGV[4], 'Role', ARGV[5], 'Username', ARGV[6],
  'Setting', ARGV[7], 'AuthVersion', ARGV[1], 'CacheSchema', ARGV[8])
if ARGV[9] == '1' and redis.call('HEXISTS', KEYS[1], 'Quota') == 0 then
...
return 1`
	result, err := common.RDB.Eval(context.Background(), script,
		...
		user.AuthVersion, user.Id, user.Email, user.Status, user.Role,
