...
// RegisterScheduledSystemTasks wires the periodic channel test and async task
// polling (video) jobs into the system task framework so a DB lease dedups
...
// asyncTaskPollHandler runs one async-task (video) polling pass per
