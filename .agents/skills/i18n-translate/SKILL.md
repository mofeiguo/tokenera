...
- Why this is mandatory, not optional:
  - Hand-editing reliably drops one of the two locales (`en`, `zh`), leaving keys missing in some languages.
...
2. Add the key via `add-missing-keys.mjs`, populating its `newKeys` object for every supported locale: `en`, `zh`. Even one key goes through the script; do not hand-edit the JSON.
...
- For languages that often expand relative to English, prefer natural but compact wording.
