...
function readThemeChartColors(): string[] {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    return []
  }

  const styles = getComputedStyle(document.documentElement)
  return CHART_TOKEN_VARS.map((name) =>
    styles.getPropertyValue(name).trim()
  ).filter(Boolean)
}

function cycleColors(palette: string[], domainLength: number): string[] {
  if (palette.length === 0) return []
  const count = Math.max(domainLength, 1)
  const first = palette.at(0)
  if (!first) return []
  return Array.from({ length: count }, (_, index) => {
    return palette.at(index % palette.length) ?? first
  })
}
...
export function getDashboardChartColors(domainLength: number): string[] {
  ...
  if (themeColors.length > 0) {
    return cycleColors(themeColors, domainLength)
  }
...
export function processChartData(
  ...
  const otherTooltipColor =
    typeof otherColor === 'string'
      ? otherColor
      : (getDashboardChartColors(1)[0] ?? '#888888')
  const modelColor = {
    type: 'ordinal',
...
export function processUserChartData(
  ...
  const { config } = getCurrencyDisplay()
  const quotaPerUnit = config.quotaPerUnit
  const userColors = getDashboardChartColors(Math.max(limit, 5))
  ...
  const userColorMap = topUsers.reduce<Record<string, string>>(
    (acc, user, i) => {
      const color = userColors.at(i % userColors.length)
      if (color) acc[user] = color
      return acc
    },
    {}
  )
