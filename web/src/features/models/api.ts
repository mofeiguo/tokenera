...
export async function getModelBindings(id: number): Promise<{
  success: boolean
  message?: string
  data: ModelChannelBinding[]
}> {
  const res = await api.get(`/api/models/${id}/bindings`)
  return res.data
}

export async function updateModelBindings(
  id: number,
...
export async function ensureCatalogModels(modelNames: string[]): Promise<{
  success: boolean
  message?: string
  data?: {
    created: string[]
  ...
  const res = await api.post('/api/models/ensure', {
